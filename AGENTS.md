...
- `docs/games/.pages` - ゲーム一覧のナビゲーション設定（新規ゲーム追加時に更新必須）
- `mkdocs.yml` - サイト全体設定（プラグインやテーマ設定）
...
4. `docs/games/.pages` の `nav` セクションに追加する
5. `python scripts/export_text.py` を実行して `.txt` ファイルを生成する
6. 作成・更新した全てのファイル（`.md`, `index.md`, `.pages`, `.txt`）をユーザーに提示する
...
- **必須構造の確認**:
  ...
  - タブ構造（`=== "SR"`, `=== "OR"`, セットアップ枠：`=== "セットアップ / 早見"` または `=== "会社の種類 / 準備"` + `=== "早見表"`）が正しく記述されているか
...
- **構成の一貫性チェック**:
  - タブの分け方が既存ファイルと一貫しているか（SR/OR/セットアップ枠：`セットアップ / 早見` または `会社の種類 / 準備` + `早見表`）
  - 見出しの階層が適切か（`##` と `###` の使い分け）
  - 内容が適切なタブに配置されているか（例：株式ラウンドのルールがSRタブにあるか）
  - プライベート一覧、列車一覧などが**セットアップ枠タブ**（`セットアップ / 早見` または `会社の種類 / 準備` / `早見表`）にあるか（ない場合も許容・オプション）
  - 「終了条件」セクションはオプション扱いであること
...
7. [ ] 新規ゲームの場合、`index.md` と `docs/games/.pages` を更新
...
3. **完全な作業実行**: 
   ...
   - 新規ゲーム追加時は `index.md` と `docs/games/.pages` も更新
   - 関連する全てのファイルを漏れなく処理
...
- `mkdocs.yml` - サイト全体設定（テーマ、プラグイン等）。変更時は必ずユーザーに確認を求める
- `docs/games/.pages` - ゲーム一覧ナビゲーション定義（新規ゲーム追加時の `nav` 更新は除く）
